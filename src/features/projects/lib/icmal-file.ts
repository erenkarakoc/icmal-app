...
const offer = z.strictObject({
  ...
  fixedRows: z
    .array(z.strictObject({ rowId: id, amount: decimal }))
    .max(10000)
    .optional(),
});
...
/**
 * Hesap/yuvarlama yöntemi sürümü (K-10 · TEMEL-05).
 *
 * Dosya SATIR TUTARLARINI saklamaz; miktar ve birim fiyattan her açılışta
 * yeniden hesaplar. Bu, yuvarlama kuralı değiştiğinde eski bir projenin
 * **sessizce farklı sayılar göstermesi** demektir. Bu damga, bir projenin
 * hangi kural kümesiyle fiyatlandığını kaydeder ki fark tespit edilebilsin.
 *
 * 1 = K-10 sözleşmesi: satır tutarı 2 ondalık yarımda sıfırdan uzağa, kalemler
 * toplamı yuvarlanmış satırlardan, gider ve teklif payları kuruşta, teklif
 * artığı en büyük kalan yöntemiyle dağıtılır (2026-09-08).
 *
 * Kural her değiştiğinde ARTTIRILMALIDIR.
 */
export const HESAP_SURUMU = 1;
export const projectSchema = z
  .strictObject({
    format: z.literal('icmal'),
    ...
    // Istege bagli: bu damgadan once yazilmis dosyalarda bulunmaz. Yoklugu
    // "bilinmiyor" demektir, "guncel" degil.
    calcVersion: z.number().int().positive().optional(),
  })
  .superRefine((project, ctx) => {
    for (const key of ['costRows', 'percentageRows'] as const) {
      ...
      project[key].forEach((row, index) => {
        if (ids.has(row.id))
          ctx.addIssue({
            code: 'custom',
            message: 'Yinelenen satır kimliği',
            path: [key, index, 'id'],
          });
        ids.add(row.id);
      });
    ...
    project.expenses.forEach((expense, index) => {
      if (expenseIds.has(expense.id)) {
        ctx.addIssue({
          code: 'custom',
          message: 'Yinelenen gider kimliği',
    ...
    project.expenses.forEach((expense, index) => {
      (expense.baseExpenseIds ?? []).forEach((baseId, baseIndex) => {
        if (!expenseIds.has(baseId)) {
          ctx.addIssue({
            code: 'custom',
            message: 'Taban gideri bulunamadı',
    ...
    (project.offer?.fixedRows ?? []).forEach((fixed, index) => {
      if (!costIds.has(fixed.rowId)) {
        ctx.addIssue({
          code: 'custom',
          message: 'Sabitlenen teklif satırı bulunamadı',
          path: ['offer', 'fixedRows', index, 'rowId'],
        });
      }
    });
    if (project.updatedAt < project.createdAt)
      ctx.addIssue({ code: 'custom', message: 'Geçersiz kayıt tarihi' });
  });
...
export async function decodeProject(bytes: Uint8Array): Promise<IcmalProject> {
  ...
  try {
    ...
    const upgraded =
      value?.format === 'icmal' && value.version === 1
    ...
    if (
      ...
      (error.message.includes('boyut sınırını') || error.message.includes('sürümü desteklenmiyor'))
    )
      throw error;
