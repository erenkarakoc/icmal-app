...
type Session = {
  costRows: CostRow[];
  setCostRows: Dispatch<SetStateAction<CostRow[]>>;
  percentageRows: PercentageCostRow[];
  setPercentageRows: Dispatch<SetStateAction<PercentageCostRow[]>>;
  giderler: Gider[];
  setGiderler: Dispatch<SetStateAction<Gider[]>>;
  /** Teklif hesabi yapilmamis projede null kalir (K-06 Soru 7). */
  teklifYontemi: KarYontemi | null;
  setTeklifYontemi: Dispatch<SetStateAction<KarYontemi | null>>;
  /** Kullanicinin sabitledigi kalem teklif tutarlari: satir kimligi -> tutar. */
  sabitTeklifler: Record<string, string>;
  setSabitTeklifler: Dispatch<SetStateAction<Record<string, string>>>;
  project: IcmalProject | null;
  setProject: Dispatch<SetStateAction<IcmalProject | null>>;
  name: string;
  setName: Dispatch<SetStateAction<string>>;
  token: string | undefined;
  setToken: Dispatch<SetStateAction<string | undefined>>;
  /** Acik proje hesaba kayitliysa kimligi; yerel dosyada undefined kalir. */
  hesapProjeId: string | undefined;
  setHesapProjeId: Dispatch<SetStateAction<string | undefined>>;
  generation: number;
  ...
  useEffect(() => {
    const prevent = (event: BeforeUnloadEvent) => {
      ...
      const pendingInput =
        ...
        !!focused.closest('[data-project-editor]') &&
        focused.value !== focused.defaultValue;
