...
import { createProject, encodeProject, HESAP_SURUMU, MAX_PROJECT_BYTES } from '../lib/icmal-file';
...
type Props =
  | { kind: 'cost'; rows: CostRow[]; onOpen: (rows: CostRow[]) => void }
...
export function ProjectFileToolbar(props: Props) {
  ...
  async function newLocalProject() {
    ...
    try {
      ...
      if (
        activeSession.current.dirty &&
        !window.confirm(
          'Kaydedilmemiş değişiklikler var. Yeni proje açıp mevcut çalışmayı değiştirmek istiyor musunuz?',
        )
      )
        return;
      ...
      if (activeSession.current.fingerprint !== previousFingerprint) {
        setMessage(
          desktop
            ? 'Yeni dosya oluşturuldu. Mevcut çalışma değiştiği için açık tutuldu; yeni dosyayı Dosya aç ile açabilirsiniz.'
            : 'Mevcut çalışma değiştiği için yeni proje açılmadı.',
        );
        return;
      }
      session.setCostRows([]);
      session.setPercentageRows([]);
      session.setGeneration((n) => n + 1);
      session.setToken(token);
      setProject(created);
      setName(created.name);
      session.setBaseline(
        desktop ? JSON.stringify({ name: created.name, costRows: [], percentageRows: [] }) : '',
      );
      props.onOpen([]);
      setMessage(desktop ? 'Yeni yerel proje oluşturuldu.' : 'Yeni proje açıldı.');
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Yeni proje oluşturulamadı.');
    } finally {
      lock.current = false;
      setBusy(false);
    }
  ...
  async function openFile(
    file?: File,
    handover?: { bytes: Uint8Array; token: string },
  ): Promise<boolean> {
    ...
    try {
      ...
      if (handover) {
        ...
        if (!file!.name.toLowerCase().endsWith('.icmal'))
          throw new Error('Lütfen bir .icmal dosyası seçin.');
      ...
      const yuklendi = await projeYukle(bytes, {
        ...
        onRows: (cost, percentage) => {
          if (props.kind === 'cost') props.onOpen(cost);
          else props.onOpen(percentage);
        },
      });
  ...
  async function exportFile(saveAs = false) {
    ...
    try {
      ...
      // Kaydedilen tutarlar GUNCEL kurallarla hesaplandi; damga da guncel olur.
      const snapshot = {
        ...base,
        calcVersion: HESAP_SURUMU,
        name,
        updatedAt: new Date().toISOString(),
        costRows: storeCostRows(session.costRows),
        percentageRows: storePercentageRows(session.percentageRows),
      ...
      anchor.href = url;
      anchor.download = `${name.trim().replace(/[<>:"/\\|?*\u0000-\u001f]/g, '_') || 'proje'}.icmal`;
      document.body.appendChild(anchor);
      anchor.click();
      anchor.remove();
      window.setTimeout(() => URL.revokeObjectURL(url), 60000);
      setProject(snapshot);
      setMessage('İndirme başlatıldı.');
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Dosya kaydedilemedi.');
    } finally {
      lock.current = false;
      setBusy(false);
    }
  ...
  async function hesabaKaydet() {
    ...
    try {
      ...
      const ozet = {
        ...
        toplamTutar: session.costRows.length
          ? calculateGrandTotal(session.costRows).toFixed(2)
          : null,
      };
      ...
      setProject(snapshot);
      setMessage(
        session.hesapProjeId ? 'Hesaba kaydedildi.' : 'Hesaba yeni proje olarak kaydedildi.',
      );
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Hesaba kaydedilemedi.');
    } finally {
      lock.current = false;
      setBusy(false);
    }
  ...
  const saveAction = useRef(exportFile);
  saveAction.current = exportFile;
  const openAction = useRef(openFile);
  openAction.current = openFile;
  ...
  useEffect(() => {
    ...
    const collect = async () => {
      ...
      if (pending && !cancelled)
        await openAction.current(undefined, { bytes: pending.bytes, token: pending.token });
    };
    void collect();
    const stop = api.onFilePending((kind) => {
      if (kind === 'icmal') void collect();
    });
    return () => {
      cancelled = true;
      stop();
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 border-b px-4 py-2">
      ...
      <input
        ...
        type="file"
        accept=".icmal"
        aria-label="İcmal dosyası"
        className="hidden"
        onChange={(e) => void openFile(e.target.files?.[0])}
      />
      <Button variant="outline" disabled={busy} onClick={() => void newLocalProject()}>
        {desktop ? 'Yeni yerel proje' : 'Yeni proje'}
      </Button>
      <Button
        variant="outline"
        disabled={busy}
        onClick={() => (desktop ? void openFile() : picker.current?.click())}
      >
        Dosya aç
      </Button>
      {desktop && (
      ...
      <Button variant="outline" disabled={busy || !name.trim()} onClick={() => void exportFile()}>
        {desktop ? 'Kaydet' : 'Dışa aktar'}
      </Button>
      {desktop && (
        <Button
          variant="outline"
      ...
      <Button variant="outline" disabled={busy || !name.trim()} onClick={() => void hesabaKaydet()}>
        Hesaba kaydet
      </Button>
      {session.dirty && (
        <span className="text-muted-foreground text-sm">Kaydedilmemiş değişiklikler</span>
      )}
      {busy && (
